...
export default function CreateCoursePage() {
  ...
  const [assignments, setAssignments] = useState([{ title: "", description: "", dueDate: "", maxScore: 10, xpReward: 100 }]);
  ...
  const handleRemoveQuestion = (quizIndex: number, qIndex: number) => {
    const newQuizzes = [...quizzes];
    newQuizzes[quizIndex].questions = newQuizzes[quizIndex].questions.filter((_, i) => i !== qIndex);
    setQuizzes(newQuizzes);
  ...
  useEffect(() => {
    ...
    // Only advance to next phase when the CURRENT phase has begun streaming
    // AND the next phase's content has appeared in the partial object.

    if (streamPhase === 'lessons') {
      const lessonStarted = !!object.lessonContent?.text?.length;
      // Advance to quizzes if quiz was requested AND quiz data appears after lesson started
      if (genConfig.generateQuiz && lessonStarted && object.quiz) {
        setStreamPhase('quizzes');
        setActiveTab('quizzes');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      }
      // Or advance directly to assignments if no quiz was requested
    ...
    // If phase is 'assignments' — we're already on the last tab, no more switching needed

    // Auto-scroll to follow live content using a dedicated anchor
    if (typeof window !== 'undefined' && isGeneratingAI && object) {
      // We use a small timeout to ensure the DOM has updated with the latest streamed text
      // before we try to scroll to the new bottom.
  ...
  const handleAIGenerate = (lessonIdx: number, config: any) => {
    ...
    // Determine the first tab to show based on what was requested (fixed priority order)
    const firstPhase: 'lessons' | 'quizzes' | 'assignments' =
      generateLesson ? 'lessons' : generateQuiz ? 'quizzes' : 'assignments';

    // Clean up default untitled placeholders before AI adds the real ones
    if (generateQuiz) {
      setQuizzes((prev) => prev.filter((q) => q.title?.trim() !== ''));
    }
    if (generateAssignment) {
      setAssignments((prev) => prev.filter((a) => a.title?.trim() !== ''));
    }
    ...
    // Start on the first requested tab so user sees content streaming immediately
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...courseInfo,
        tags: courseInfo.tags.split(",").map(t => t.trim()).filter(Boolean),
        lessons,
        quizzes,
        assignments
      };
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Design a premium IT Training experience</p>
            </div>
          </div>
          <button
                  ...
                  {lessons.map((lesson, idx) => {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {expandedLessonIdx === idx ? <ChevronUp className="w-5 h-5 text-muted-foreground" /> : <ChevronRight className="w-5 h-5 text-muted-foreground" />}
                      </div>
                    </div>
                              ...
                              <div className="md:col-span-6 flex flex-col gap-1.5">
                                <label className="text-[10px] font-bold text-muted-foreground uppercase">Lesson Title</label>
                                 <input
                                  ...
                                  value={lesson.title || ""}
                                  readOnly={isGeneratingAI && generatingLessonIdx === idx}
                                  onChange={(e) => {
                                    if(isGeneratingAI && generatingLessonIdx === idx) return;
          ...
          {activeTab === "assignments" && (
            <div className="flex flex-col gap-6">
              ...
              <div className="flex justify-between mt-8 pt-4 border-t border-border">
                ...
                <div className="flex items-center gap-4">
                   <p className="text-xs text-muted-foreground italic hidden sm:block">Ready to publish your course?</p>
                   <button
                    onClick={handleSubmit}
                    disabled={isSubmitting}
                  ...
                  >
                    ...
                    {isSubmitting ? "Processing..." : "Finish & Create Course"}
                  </button>
                </div>
              </div>
            </div>
