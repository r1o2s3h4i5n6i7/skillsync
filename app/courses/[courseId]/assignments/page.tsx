...
export default function CourseAssignmentsPage() {
  ...
  const { getCourse, getCourseAssignments, enrolledIds } = useCourses();
  const { user } = useAuth();
  ...
  const [assignments, setAssignments] = useState<any[]>(
    getCourseAssignments(courseId).map(a => ({ ...a, status: a.status || "PENDING" }))
  );
  ...
  if (!canAccess) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <p className="text-muted-foreground text-center">You must enroll in this course to access assignments.</p>
