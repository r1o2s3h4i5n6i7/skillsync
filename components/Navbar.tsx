...
export default function Navbar() {
  ...
  const notifColors: Record<string, string> = {
    ...
    COURSE_PUBLISHED: "from-emerald-400 to-green-500",
    STUDENT_ENROLLED: "from-blue-400 to-cyan-500",
    SYSTEM_ALERT: "from-amber-400 to-red-500",
    MEMBER_JOINED: "from-indigo-400 to-purple-500",
  };
  ...
  return (
    <nav className="sticky top-0 z-40 w-full h-16 bg-card/90 backdrop-blur-xl border-b border-border flex items-center px-4 sm:px-6 gap-3 shadow-sm">
      ...
      <Link href={getDashboardLink()} className="flex items-center gap-2.5 shrink-0 select-none">
        <div className="w-9 h-9 rounded-xl brand-gradient flex items-center justify-center shadow-md shadow-indigo-500/30">
          <Brain className="w-5 h-5 text-white" />
        </div>
        <span className="text-lg font-extrabold hidden sm:block">
          <span className="brand-gradient-text text-indigo-600">SKILL</span>
            ...
            <AnimatePresence>
              {showNotifs && (
                <motion.div
                  ...
                  className="absolute right-0 mt-2 w-80 bg-card border border-border rounded-2xl shadow-2xl shadow-pink-500/10 overflow-hidden z-50"
                >
